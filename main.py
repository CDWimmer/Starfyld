...
# fix for dpi scaling on UI scaled monitors e.g. laptops. Might need to revisit this.
...
class App(pyglet.window.Window):
    def __init__(self, width, height, *args, **kwargs):
        ...
        self.stars: List[pyglet.shapes.Circle] = [
            make_random_circle(self.left, self.right, self.top, self.bottom, self.main_batch) for i in range(NUM_STARS)]
        ...
        for i in range(1, NUM_LINES + 1):
            ...
            self.static_grid.append(
                pyglet.shapes.Line(x=from_left,
                                   y=self.bottom,
                                   x2=from_left,
                                   y2=self.top,
                                   width=0.5,
        ...
        for i in range(1, int(self.height / separation) + 1):
            ...
            self.static_grid.append(
                pyglet.shapes.Line(x=self.left,
                                   y=from_bottom,
                                   x2=self.right,
                                   y2=from_bottom,
                                   width=0.5,
        ...
        self.radar_container = pyglet.shapes.BorderedRectangle(
            ...
            height=-200,
            border=1,
            color=(int(GRID_COLOUR[0]*0.3), int(GRID_COLOUR[1]*0.3), int(GRID_COLOUR[2]*0.3), int(GRID_COLOUR[3])),
    ...
    def wind_label_update(self):
        self.additional_wind_label.text = f"Additional wind: {self.additional_wind_horizontal:+}, {self.additional_wind_vertical:+}"
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        match symbol:
            case key.UP:
                self.additional_wind_vertical += 1 * (10 if modifiers & key.MOD_SHIFT else 1)
                self.wind_label_update()
            case key.DOWN:
                self.additional_wind_vertical -= 1 * (10 if modifiers & key.MOD_SHIFT else 1)
                self.wind_label_update()
            case key.LEFT:
                self.additional_wind_horizontal -= 1 * (10 if modifiers & key.MOD_SHIFT else 1)
                self.wind_label_update()
            case key.RIGHT:
                self.additional_wind_horizontal += 1 * (10 if modifiers & key.MOD_SHIFT else 1)
                self.wind_label_update()
    ...
    def generate_new_star(self, heading_horizontal, heading_vertical):
        horizontal_or_vertical = random.choice(["horizontal", "vertical"])  # good enough
        match horizontal_or_vertical:
            case "horizontal":
                ...
                if heading_horizontal >= 0:  # stars moving right
                    ...
                    lim_right = self.left - SPAWN_BOARDER_LIMIT  # n pixels out of screen on left
                else:  # stars moving left
                    # print("horizontal <-")
                    lim_left = self.right + SPAWN_BOARDER_LIMIT  # n pixels out of screen on right
            ...
            case "vertical":
                ...
                if heading_vertical >= 0:  # stars moving up
                    ...
                    lim_bottom = self.bottom - SPAWN_BOARDER_LIMIT  # n pixels out of screen on bottom
                else:
                    # print("vertical v")
                    lim_top = self.top + SPAWN_BOARDER_LIMIT  # n pixels out of screen on top
            ...
            case _:
                ...
                lim_bottom = self.bottom
                lim_top = self.top
        new_star = make_random_circle(lim_left, lim_right, lim_bottom, lim_top, self.main_batch)
...
target_monitor_number: int = int(input(f"Pick a display number: {int_list_of_monitors}: "))

app = App(width=monitors[target_monitor_number].width,
...
def update(dt):
    app.time += dt * 0.1
    travel_vertical_multiplier = opensimplex.noise2(app.time, 0) + (app.additional_wind_vertical*0.1)
    travel_horizontal_multiplier = opensimplex.noise2(0, app.time) + (app.additional_wind_horizontal*0.1)
    for star in app.stars:
        # check if star needs replacing due to being off-screen
        if star.x < (app.left - DELETE_BOARDER_LIMIT) or star.x > (app.right + DELETE_BOARDER_LIMIT) or \
                star.y < (app.bottom - DELETE_BOARDER_LIMIT) or star.y > (app.top + DELETE_BOARDER_LIMIT):
            star.delete()
            app.stars.remove(star)
            app.generate_new_star(travel_horizontal_multiplier, travel_vertical_multiplier)
            continue
    ...
    pointer_radius = app.radar_radius  #* abs(travel_vertical_multiplier) * abs(travel_horizontal_multiplier)
